...
import type { IncomingMessage, SignupIncomingMessage } from "common/index";
import { prismaclient } from "db/client";
import { PublicKey } from "@solana/web3.js";
import nacl from "tweetnacl";
import nacl_util from "tweetnacl-util";
import { WebsiteTickStatus } from "@prisma/client";
    ...
    websocket: {
        async message(ws: ServerWebSocket<unknown>, message: string) {
            ...
            if (data.type === 'signup') {
                ...
                const callbacks = CALLBACKS[data.data.callbackId]
                if(callbacks) {
                    callbacks(data);
                    delete CALLBACKS[data.data.callbackId];
                } else {
                    console.warn("Unknown callbackId received:", data.data.callbackId);
                }
            }
        },
                    ...
                    await prismaclient.$transaction(async (tx) => {
                        await tx.websiteTick.create({
                            data: {
                                ...
                                latency,
                                createdAt: new Date(),
                                status: status === 'Good' ? WebsiteTickStatus.GOOD : WebsiteTickStatus.BAD,
                            },
                        });
